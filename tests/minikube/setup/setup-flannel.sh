#!/bin/bash
# Setup script for Minikube with Flannel CNI
...
install_flannel() {
    ...
    print_info "Downloading Flannel manifest..."
    curl -L https://github.com/flannel-io/flannel/releases/download/${FLANNEL_VERSION}/kube-flannel.yml -o kube-flannel.yml
    
    # Apply Flannel manifest
    print_info "Applying Flannel manifest..."
    ...
    print_info "Flannel installed successfully."
}

# Verify Flannel installation
verify_flannel() {
    print_header "Verifying Flannel installation"
    
    # Check Flannel pods
    print_info "Checking Flannel pods..."
    ...
    print_info "Flannel verification completed."
}
...
main() {
    ...
    print_info "Minikube with Flannel CNI is now ready for testing."
    print_info "You can now run the CNI Migration Tool to migrate from Flannel to Cilium."
}
